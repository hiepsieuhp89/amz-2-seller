  ...
  if (isError)
    return (
      <div className="text-center p-8 bg-red-50 rounded-lg border border-red-200 shadow-sm">
        <h4 className="text-lg font-semibold text-red-600">Có lỗi xảy ra khi tải dữ liệu</h4>
    ...
    <div className="min-h-screen py-8 px-4 bg-gray-50">
      <div className="max-w-7xl mx-auto">
        ...
        <div className="mb-10 text-center">
          <h2 className="text-3xl font-bold mb-4 text-gray-800">Gói Quảng Bá Cửa Hàng</h2>
          <p className="text-base text-gray-600 max-w-3xl mx-auto">
            Tăng khả năng hiển thị và tiếp cận khách hàng tiềm năng cho sản phẩm của bạn với các gói quảng bá hiệu quả.
            Mỗi gói cung cấp các lợi ích khác nhau để phù hợp với nhu cầu kinh doanh của bạn.
          </p>
        </div>
              ...
              {sortedPackages.map((pkg, index) => (
                ...
                >
                  ...
                  <div className={`absolute top-4 right-4 z-10 py-1 px-3 rounded-full text-xs font-semibold text-white bg-gradient-to-r ${getPriceGradient(pkg.price, index)}`}>
                    {index === 0 ? "Cơ bản" : index === 1 ? "Nâng cao" : "Cao cấp"}
                  </div>
                  
                  {/* Header with image */}
                  <div className={`w-full h-44 relative bg-gradient-to-br ${getPriceGradient(pkg.price, index)}`}>
