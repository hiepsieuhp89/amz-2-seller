...
export const updateBank = async (payload: IUpdateBank): Promise<IAuthResponse> => {
...
export const updateUser = async (payload: IUpdateUser): Promise<IAuthResponse> => {
  const res = await sendPut('/auth/update-user', payload);
  const data: IAuthResponse = res;
  return data;
};

export const changePassword = async (payload: IChangePassword): Promise<IAuthResponse> => {
